...
append_unique_fragment() {
    ...
    eval "local -a target_array=(\"\${$output_array_name[@]}\")"
    fragment_in_list "$fragment" "${target_array[@]}" && return 0
    eval "$output_array_name+=(\"$fragment\")"
}
...
rename_firmware_artifacts() {
    ...
    sanitized_name=$(echo "$device_name" | tr '[:upper:]' '[:lower:]' | tr '/:' '-_' | sed 's/[^a-z0-9._-]/_/g')
    ...
    for file in "$firmware_dir"/*; do
        ...
        case "$base_name" in
            *.manifest|*.sha256sum|*.md5|*.sig)
                continue
                ;;
        esac

        new_name="${sanitized_name}-${base_name}"
        if [[ "$base_name" != "$new_name" ]]; then
            mv -f "$file" "$firmware_dir/$new_name"
        fi
    done
}
...
rename_firmware_artifacts "$FIRMWARE_DIR" "$Dev"
